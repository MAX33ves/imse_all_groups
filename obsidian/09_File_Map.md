...
| `03_outputs/tables/training_pool_suspension_model_comparison.csv` | 悬挂类型候选模型比较 / suspension candidate model comparison |
| `03_outputs/tables/training_pool_suspension_selected_cv_predictions.csv` | 悬挂类型选中模型 CV 预测 / selected suspension CV predictions |
| `03_outputs/tables/training_pool_suspension_cv_confusion.csv` | 悬挂类型 CV 混淆矩阵 / suspension CV confusion matrix |
| `03_outputs/models/training_pool_suspension_selection_summary.json` | 悬挂类型模型选择摘要 / suspension model-selection summary |
| `03_outputs/models/training_pool_suspension_final_model.pkl` | 悬挂类型最终模型 / final suspension model |
