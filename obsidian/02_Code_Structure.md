...
| `04b_bike_type_classification.py` | 组级交叉验证并选择单车类型分类模型 / performs group-level CV and selects the bike-type classifier |
...
| `bike_type_modeling.py` | 单车类型分类候选、防泄漏输入空间、组级 CV、最终训练 / bike-type classifier candidates, leakage-safe input spaces, group CV, and final training |
