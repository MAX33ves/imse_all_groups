...
项目现在包含两个监督学习任务：第一是根据骑行过程中的传感器信号、单车类型和 rider weight 预测连续胎压 `pressure_bar`；第二是只根据 Sagemotion 信号特征预测单车类型 `FAT`、`ISY` 或 `MTB`。

The project now contains two supervised learning tasks. The first predicts continuous tire pressure `pressure_bar` from riding sensor features, bike type, and rider weight. The second predicts bike type, `FAT`, `ISY`, or `MTB`, from Sagemotion signal features only.
...
Step 04b 对单车类型分类模型使用同样的 leave-one-group-out 逻辑，但输入规则更严格：不使用 `bike`、`pressure_bar`、`rider_weight_kg`、`group`、`run_id` 或文件名，只使用 Sagemotion 信号特征。

Step 04b applies the same leave-one-group-out logic to the bike-type classifier, but with stricter input rules: it excludes `bike`, `pressure_bar`, `rider_weight_kg`, `group`, `run_id`, and file names, using only Sagemotion signal features.
...
单车类型模型是 `bike_type_ffnn_ensemble_ens3_signal_full_pca10_tanh_h6_a1`，结构为 `StandardScaler -> PCA(n_components=10) -> MLPClassifier(hidden_layer_sizes=(6,), activation='tanh')`，并使用 3 个随机种子模型做 ensemble。

The bike-type model is `bike_type_ffnn_ensemble_ens3_signal_full_pca10_tanh_h6_a1`. Its structure is `StandardScaler -> PCA(n_components=10) -> MLPClassifier(hidden_layer_sizes=(6,), activation='tanh')`, also using a 3-seed ensemble.
...
单车类型分类模型的 run-level CV accuracy 为 0.986，macro-F1 为 0.986。这个结果说明当前信号特征对区分 `FAT`、`ISY` 和 `MTB` 很有效，但它仍然是本地 CV 结果，不是老师隐藏测试集结果。

The bike-type classifier reaches 0.986 run-level CV accuracy and 0.986 macro-F1. This indicates that the current signal features separate `FAT`, `ISY`, and `MTB` very well, but it is still a local CV result rather than an instructor hidden-test result.

## 8. 推荐复跑方式 / Recommended Re-run Method

在 VSCode 打开 `C:\Users\user\Desktop\IMSE\imse_all_groups`，运行任务 `Training Pool Run Full Pipeline`。也可以按照 README 逐步运行 Step 00 到 Step 07，其中 Step 04b 会生成单车类型分类模型，Step 07 会生成老师视角审查补充报告。

Open `C:\Users\user\Desktop\IMSE\imse_all_groups` in VSCode and run the task `Training Pool Run Full Pipeline`. You can also follow the README and run Steps 00 to 07 one by one. Step 04b generates the bike-type classifier, and Step 07 generates the teacher-review supplement.
