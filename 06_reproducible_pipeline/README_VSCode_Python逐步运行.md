...
| Suspension target / 悬挂类型预测目标 | `Suspension because of tyres`, `No Suspension`, or `Front and rear Suspension` / `Suspension because of tyres`、`No Suspension` 或 `Front and rear Suspension` |
| Suspension-model inputs / 悬挂模型输入 | Sagemotion signal features only; no `bike`, `suspension_type`, `pressure_bar`, `rider_weight_kg`, or split metadata / 只使用 Sagemotion 信号特征；不使用 `bike`、`suspension_type`、`pressure_bar`、`rider_weight_kg` 或划分元数据 |
...
| Suspension model comparison / 悬挂类型模型比较 | `03_outputs/tables/training_pool_suspension_model_comparison.csv` |
| Suspension selected CV predictions / 悬挂类型选中模型 CV 预测 | `03_outputs/tables/training_pool_suspension_selected_cv_predictions.csv` |
| Suspension CV confusion matrix / 悬挂类型 CV 混淆矩阵 | `03_outputs/tables/training_pool_suspension_cv_confusion.csv` |
| Suspension final model file / 悬挂类型最终模型文件 | `03_outputs/models/training_pool_suspension_final_model.pkl` |
...
| Suspension classifier report / 悬挂类型分类报告 | `04_report/training_pool_suspension_classifier_report_bilingual.md` |
